...
cdef int read_thread(void *obj_id) with gil:
    cdef VideoState vs = <VideoState>obj_id
    with nogil:
        return vs.read_thread()
cdef int video_thread(void *obj_id) with gil:
    cdef VideoState vs = <VideoState>obj_id
    with nogil:
        return vs.video_thread()
cdef int subtitle_thread(void *obj_id) with gil:
    cdef VideoState vs = <VideoState>obj_id
    with nogil:
...
cdef void free_subpicture(SubPicture *sp) nogil:
    avsubtitle_free(&sp.sub)

cdef void pp(const char * text) nogil:
...
cdef class VideoState(object):
    ...
    cdef void cInit(VideoState self, MTGenerator mt_gen, char *input_filename,
        ...
        av_strlcpy(self.filename, input_filename, sizeof(self.filename))
        self.iformat = file_iformat
        self.ytop = 0
        self.xleft = 0
    
        # start video display
        self.pictq_mutex = mt_gen.create_mutex()
        self.pictq_cond  = mt_gen.create_cond()
    
        self.subpq_mutex = mt_gen.create_mutex()
        self.subpq_cond  = mt_gen.create_cond()
     
        self.continue_read_thread = mt_gen.create_cond()
        ...
        self.av_sync_type = av_sync_type
        
        #with gil:
        self.read_tid = mt_gen.create_thread(read_thread, self.self_id)
        if self.read_tid.thread == NULL:
            with gil:
    ...
    def __dealloc__(VideoState self):
        ...
        self.read_tid.wait_thread(self.read_tid.thread, NULL)
        #del self.videoq
        #del self.audioq
        #del self.subtitleq
        ...
        self.mt_gen.destroy_mutex(self.pictq_mutex)
        self.mt_gen.destroy_cond(self.pictq_cond)
        self.mt_gen.destroy_mutex(self.subpq_mutex)
        self.mt_gen.destroy_cond(self.subpq_cond)
        self.mt_gen.destroy_cond(self.continue_read_thread)
        IF not CONFIG_AVFILTER:
    ...
    cdef void stream_seek(VideoState self, int64_t pos, int64_t rel, int seek_by_bytes) nogil:
        if not self.seek_req:
            ...
            if seek_by_bytes:
                self.seek_flags |= AVSEEK_FLAG_BYTE
            self.seek_req = 1
            self.continue_read_thread.cond_signal(self.continue_read_thread.cond)
    ...
    cdef void pictq_next_picture(VideoState self) nogil:
        ...
        self.pictq_mutex.lock_mutex(self.pictq_mutex.mutex)
        self.pictq_size -= 1
        self.pictq_cond.cond_signal(self.pictq_cond.cond)
        self.pictq_mutex.unlock_mutex(self.pictq_mutex.mutex)
    ...
    cdef void video_refresh(VideoState self, double *remaining_time) nogil:
        ...
        if (not self.player.display_disable) and self.show_mode != SHOW_MODE_VIDEO and self.audio_st != NULL:
            time = av_gettime() / 1000000.0
            if self.force_refresh or self.last_vis_time + self.player.rdftspeed < time:
                self.video_display()
                self.last_vis_time = time
            remaining_time[0] = FFMIND(remaining_time[0], self.last_vis_time + self.player.rdftspeed - time)
        if self.video_st != NULL:
            redisplay = 0
                ...
                if state == retry:
                    if self.pictq_size == 0:
                        ...
                        if time < self.frame_timer + delay and not redisplay:
                            remaining_time[0] = FFMIND(self.frame_timer + delay - time, remaining_time[0])
                            return
                        ...
                        if self.pictq_size > 1:
                            ...
                            if (not self.step) and (redisplay or self.player.framedrop > 0 or\
                        ...
                        if self.subtitle_st != NULL:
                            while self.subpq_size > 0:
                                ...
                                or (sp2 != NULL and self.vidclk.pts > (sp2.pts + <float> sp2.sub.start_display_time / 1000.)):
                                    ...
                                    self.subpq_mutex.lock_mutex(self.subpq_mutex.mutex)
                                    self.subpq_size -= 1
                                    self.subpq_cond.cond_signal(self.subpq_cond.cond)
                                    self.subpq_mutex.unlock_mutex(self.subpq_mutex.mutex)
                                else:
                                    break
                        state = display
                        continue
                    ...
                    # display picture
                    if (not self.player.display_disable) and self.show_mode == SHOW_MODE_VIDEO:
                        self.video_display()
                    self.pictq_next_picture()
                    if self.step and not self.paused:
                        self.stream_toggle_pause()
                break
    ...
    cdef int queue_picture(VideoState self, AVFrame *src_frame, double pts,
        ...
        while (self.pictq_size >= VIDEO_PICTURE_QUEUE_SIZE - 1 and
            ...
            self.pictq_cond.cond_wait(self.pictq_cond.cond, self.pictq_mutex.mutex)
        self.pictq_mutex.unlock_mutex(self.pictq_mutex.mutex)
    ...
    IF CONFIG_AVFILTER:
        ...
        cdef int configure_audio_filters(VideoState self, const char *afilters, int force_output_format) nogil:
            ...
            ret = avfilter_graph_create_filter(&filt_asrc, avfilter_get_by_name("abuffer"),
                                               "ffplay_abuffer", asrc_args, NULL, self.agraph)
            if ret >= 0:
                ret = avfilter_graph_create_filter(&filt_asink, avfilter_get_by_name("abuffersink"),
                                                   "ffplay_abuffersink", NULL, NULL, self.agraph)
            if ret >= 0:
    ...
    cdef int subtitle_thread(VideoState self) nogil:
        ...
        while 1:
            ...
            while self.subpq_size >= SUBPICTURE_QUEUE_SIZE and not self.subtitleq.abort_request:
                self.subpq_cond.cond_wait(self.subpq_cond.cond, self.subpq_mutex.mutex)
            self.subpq_mutex.unlock_mutex(self.subpq_mutex.mutex)
...
#                     self.last_clock = is->audio_clock;
    ...
    cdef int stream_component_open(VideoState self, int stream_index) nogil:
        ...
        if avctx.codec_type == AVMEDIA_TYPE_AUDIO:
            ...
            self.video_tid = self.mt_gen.create_thread(video_thread, self.self_id)
            ...
            self.subtitle_tid = self.mt_gen.create_thread(subtitle_thread, self.self_id)
        return 0
    ...
    cdef void stream_component_close(VideoState self, int stream_index) nogil:
        ...
        if avctx.codec_type == AVMEDIA_TYPE_AUDIO:
            ...
            if self.rdft != NULL:
                av_rdft_end(self.rdft)
                av_freep(&self.rdft_data)
                self.rdft = NULL
                self.rdft_bits = 0
            IF CONFIG_AVFILTER:
                avfilter_graph_free(&self.agraph)
            ...
            self.video_tid.wait_thread(self.video_tid.thread, NULL)
            ...
            self.subtitle_tid.wait_thread(self.subtitle_tid.thread, NULL)
    ...
    cdef int read_thread(VideoState self) nogil:
        ...
        cdef MTMutex *wait_mutex = self.mt_gen.create_mutex()
        ...
        with gil:
            err = avformat_open_input(&ic, self.filename, self.iformat, &self.player.format_opts)
        if err < 0:
            if av_strerror(err, errbuf, sizeof(errbuf)) < 0:
                errbuf_ptr = strerror(AVUNERROR(err))
            av_log(NULL, AV_LOG_ERROR, "%s: %s\n", self.filename, errbuf_ptr)
            return self.failed(wait_mutex, -1)
        t = av_dict_get(self.player.format_opts, "", NULL, AV_DICT_IGNORE_SUFFIX)
        if t != NULL:
            av_log(NULL, AV_LOG_ERROR, "Option %s not found.\n", t.key)
            return self.failed(wait_mutex, AVERROR_OPTION_NOT_FOUND)
        self.ic = ic
        ...
        if self.player.window_title == NULL and t != NULL:
            self.player.window_title = av_asprintf("%s - %s", t.value, self.player.input_filename)
        ...
        if self.player.start_time != AV_NOPTS_VALUE:
            ...
            if ret < 0:
                av_log(NULL, AV_LOG_WARNING, "%s: could not seek to position %0.3f\n",
                       self.filename, <double>timestamp / <double>AV_TIME_BASE)
        ...
        if self.player.show_status:
            av_dump_format(ic, 0, self.filename, 0)
    
        self.show_mode = self.player.show_mode
        # open the streams
        if st_index[<int>AVMEDIA_TYPE_AUDIO] >= 0:
        ...
        while 1:
            ...
            if self.queue_attachments_req:
                if self.video_st != NULL and self.video_st.disposition & AV_DISPOSITION_ATTACHED_PIC:
                    ...
                    if ret < 0:
                        return self.failed(wait_mutex, ret)
            ...
            or self.subtitleq.abort_request))):
                # wait 10 ms
                wait_mutex.lock_mutex(wait_mutex.mutex)
                self.continue_read_thread.cond_wait_timeout(self.continue_read_thread.cond,
                                                            wait_mutex.mutex, 10)
                wait_mutex.unlock_mutex(wait_mutex.mutex)
                continue
            ...
            (self.video_finished == self.videoq.serial and self.pictq_size == 0)):
                if self.player.loop != 1:
                    ...
                    return self.failed(wait_mutex, AVERROR_EOF)
            if eof:
                if self.video_stream >= 0:
                    av_init_packet(pkt)
                    pkt.data = NULL
    ...
    cdef inline int failed(VideoState self, MTMutex *wait_mutex, int ret) nogil:
        ...
        if ret != 0:
            ...
            event.user.data1 = self.self_id
            SDL_PushEvent(&event)
        self.mt_gen.destroy_mutex(wait_mutex)
        return 0
    ...
    cdef void toggle_audio_display(VideoState self) nogil:
        ...
        cdef int next = <int>self.show_mode
        next = (next + 1) % <int>SHOW_MODE_NB
        while next != <int>self.show_mode and (next == <int>SHOW_MODE_VIDEO and\
        self.video_st == NULL or next != <int>SHOW_MODE_VIDEO and self.audio_st == NULL):
            next = (next + 1) % <int>SHOW_MODE_NB
        if <int>self.show_mode != next:
            fill_rectangle(self.player.screen, self.xleft, self.ytop, self.width,
